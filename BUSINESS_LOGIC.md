...
4. **Teacher payout on that lifetime free session:** if teacher still in interview **for that domain** → 0% on the whole package (existing rule). If unlocked → `TeacherEarnings` on the snapshot is reduced by the first-session teacher share (`firstTeacher = TeacherEarnings × firstSessionMinutes / TotalMinutes`); `PlatformShare = AmountDue − TeacherEarnings`. Completing the free first schedule does **not** create a `TeacherEarningLine`; later sessions accrue from the reduced `TeacherEarnings` prorated over **earnable** minutes only (`TotalMinutes − firstFreeMinutes`), source `SessionCompleted`. After the student has consumed the trial, later enrollments are full pay for the student and full teacher earnings on every session (including the first of those packages). **Interview-pending at quote:** frozen snapshot keeps `TeacherSharePct = 0` and `TeacherEarnings = 0`; Teacher/Admin APIs expose **upfront projected** earnings from snapshot hourly/minutes + starter level share (fixed at first view, not recalculated after sessions). Session accrual for paid schedules uses the same projection when snapshot earnings are zero.
5. **Cancel before first session:** Student owner may cancel `PendingPayment` (no refund) or `Active` when no schedule has started; paid enrollments get a refund via the **original** payment provider recorded on the `Payment` row (`Refund` entity + `PaymentStatus.Refunded`; list/API type is always `Refund`, not a negative payment). The **active** gateway for new intents is admin-switchable (`SystemSettings` key `Payments.Gateway`; SuperAdmin Settings UI) with env fallback `PAYMENT_PROVIDER`. Supported gateways: **Mock** (local/dev), **Moyasar**, **PayTabs**, **HyperPay**, **Stripe**. API keys stay in `.env`; only the active provider name is in the DB. Free-trial cancel sets the consumption row to **`CancelledBeforeStart`** (audit kept forever), restores `HasUsedFreeTrialSession` when no other `Reserved` consumption exists, and **reverts auto interview unlock** on `TeacherDomainPricing` when `InterviewUnlockSource = AutoFromSession` and attributed to that enrollment with no other completed sessions in the domain. **Admin-assigned** levels (`InterviewUnlockSource = Admin`) are never reverted on cancel.
...
9. **Refund clawback:** On succeeded refund, void `Pending` and `OnHold` `TeacherEarningLine`s for the enrollment (full refund voids all pending/on-hold; partial voids newest pending/on-hold until amount covered). Lines already in a **Paid** payout batch are not reversed automatically (`PayoutImpact = AlreadyPaid`).
10. **Session complaints & earnings hold:** Students may file one open complaint per session (`SessionComplaint`: reasons include teacher no-show/late, quality, technical). Blocking statuses: `Open`, `InReview`, `AwaitingTeacher`, `AwaitingStudent`. On file, any existing line for that schedule moves to `TeacherEarningLineStatus.OnHold`; on session complete with a blocking complaint, accrual is created as `OnHold` instead of `Pending`. Teacher session earnings are **not payout-eligible** while status is `OnHold` or while a blocking complaint exists. Admin resolves via `POST /Admin/Sessions/{id}/Complaints/{id}/Resolve` with optional `GET …/ResolvePreview` for finance impact. Resolution side effects:

| Resolution | Refund | Session earning | Other |
|------------|--------|-----------------|-------|
| `NoAction`, `RejectComplaint` | — | OnHold → **Pending** | — |
| `WarnTeacher` | — | OnHold → **Pending** | Audit `TeacherWarned` |
| `FullRefund`, `PartialRefund` | Session-prorated share of `AmountDue` (admin override); auto `PaymentId` lookup | **Void** complained session line | Refund clawback on other enrollment lines; `RefundId` linked on complaint |
| `DeductTeacherEarning`, `ReplacementSession` | — | **Void** complained session line | Replacement creates new `CourseSchedule` (`Scheduled`); `ReplacementScheduleId` linked |

`PayoutService` lists only `Pending` lines. Live meeting data uses LiveKit (`MeetingUrl` / live token / presence events), not Zoom.
